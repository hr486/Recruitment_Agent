...
const AnalyseResults = ({ navigateTo }) => {
  ...
  const getHighestSeverityFromEvents = (events = []) => {
    const rank = { info: 1, warning: 2, medium: 2, high: 3, critical: 4 };
    let best = 'info';
    let bestRank = 0;
    (events || []).forEach((e) => {
  ...
  const fetchViolationEvents = async (jdId, email, force = false) => {
    const emailKey = String(email || '').trim().toLowerCase();
    if (!emailKey) return;
    if (!force && violationEventsMap[emailKey]) return;
                        ...
                        {(violationEventsMap[String(cand.Email || '').toLowerCase()] || []).map((event, idx) => (
                          <div key={idx} style={{ marginBottom: '10px', paddingBottom: '10px', borderBottom: idx < (violationEventsMap[String(cand.Email || '').toLowerCase()] || []).length - 1 ? '1px solid rgba(255,255,255,0.1)' : 'none' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                              ...
                              <span style={{ fontSize: '0.8rem', color: 'var(--primary)', fontWeight: 600 }}>{event.event_type}</span>
                              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                {event.event_time ? new Date(event.event_time).toLocaleTimeString() : ''}
                              </span>
                            </div>
                            <p style={{ margin: '0', fontSize: '0.8rem', color: 'var(--text-muted)' }}>{event.details}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ) : (
                    <div style={{ padding: '12px', background: 'var(--bg-input)', borderRadius: '8px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                      No violation events recorded.
                    </div>
                  )}
                  <div style={{ background: 'var(--bg-input)', padding: '16px', borderRadius: '8px', border: `2px solid ${getViolationSeverity(cand.test_status, cand.Email).color}`, marginBottom: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                      <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>Severity Level</span>
                      <span style={{ fontSize: '1rem', fontWeight: 700, color: getViolationSeverity(cand.test_status, cand.Email).color, textTransform: 'uppercase' }}>
                        {getViolationSeverity(cand.test_status, cand.Email).level}
                      </span>
                    </div>
